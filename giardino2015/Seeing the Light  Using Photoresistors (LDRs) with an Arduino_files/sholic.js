...
function oiq_doTag() {
	...
	var req='https://px.owneriq.net/j/'+'?pt=sholic'+'&t='+encodeURI(t.join());
	if (window.oiq_pSource) req+='&s='+window.oiq_pSource;
	oiq_is(req);

	if (window.oiq_pUser) {
		var oiq_user_img = new Image();
		oiq_user_img.src = "https://px.owneriq.net/euid?pt=sholic&uid="+encodeURIComponent(window.oiq_pUser);
	}
	...
	if(oiq_parsedReferer) {
		if(oiq_parsedReferer.host && oiq_parsedReferer.protocol && oiq_parsedReferer.query) {
			if(oiq_parsedReferer.host.match(/google|bing|ask|aol/gi))	{
				...
				if(oiq_searchString) {
					var oiq_searchTagURL = 'https://px.owneriq.net/esq?pt=sholic&URL=';
